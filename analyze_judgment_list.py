...
def analyze_relevance_issues(df):
    ...
    if len(zero_click_positive) > 0:
        ...
        print(f"   Sample:")
        print(zero_click_positive[['keyword', 'product_id', 'total_impressions', 'total_clicks', 'ctr', 'relevance_grade']].head(10))
...
def analyze_keyword_quality(df):
    ...
    print(f"Sample of keywords with no relevant results:")
    print(no_relevant.head(20).to_dict())
...
def analyze_ctr_distribution(df):
    ...
    if len(very_high_ctr) > 0:
        ...
        print(very_high_ctr[['keyword', 'product_id', 'total_impressions', 'total_clicks', 'ctr', 'relevance_grade']].head(10))
...
def analyze_data_quality(df):
    ...
    if len(inconsistent_ctr) > 0:
        print(f"Sample:")
        print(inconsistent_ctr[['keyword', 'product_id', 'total_impressions', 'total_clicks', 'ctr', 'calculated_ctr']].head(5))
...
def analyze_imbalance(df):
    ...
    if 0 in keyword_grade_counts.columns and keyword_grade_counts.shape[1] > 1:
        keyword_grade_counts['imbalance_ratio'] = keyword_grade_counts[0] / keyword_grade_counts.drop(columns=[0]).sum(axis=1)
...
def create_cleaned_dataset(df, output_path):
    ...
    print(df_clean['relevance_grade'].value_counts(normalize=True).sort_index() * 100)
